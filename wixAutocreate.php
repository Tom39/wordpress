...
*/
			...
			//bm25計算
			wix_bm25( $words_countArray, $post->ID );

			// foreach ($similarityObj as $keyword => $valueArray) {
			// 	var_dump($keyword);
		...
		if ( !empty($similarityObj) ) {
			...
			foreach ($similarityObj as $keyword => $array) {
				...
				if ( empty($insertEntry) )
					$insertEntry = '(' . $doc_id . ', "' . $keyword .'", ' . $tf . ', ' . $idf . ', ' . $tf_idf . ', ' . $bm25 . '), ';
				else
					$insertEntry = $insertEntry . '(' . $doc_id . ', "' . $keyword .'", ' . $tf . ', ' . $idf . ', ' . $tf_idf . ', ' . $bm25 . '), ';
			}

			$sql = 'INSERT INTO ' . $table_name . '(doc_id, keyword, tf, idf, tf_idf, bm25) VALUES ' . $insertEntry;
	...
	//UPDATE
		...
		foreach ($similarityObj as $keyword => $array) {
			...
			if ( array_key_exists($keyword, $existing_keywordList) ) {
				$sql = 'UPDATE ' . $table_name . ' SET tf = ' . $tf . ', idf = ' . $idf . ', tf_idf = ' . $tf_idf . ', bm25 = ' . $bm25 . ' WHERE doc_id = ' . $doc_id .' AND keyword = "' . $keyword . '"';
				unset($existing_keywordList[$keyword]);
			} else {
				$sql = 'INSERT INTO ' . $table_name . '(doc_id, keyword, tf, idf, tf_idf) VALUES (' . $doc_id . ', "' . $keyword .'", ' . $tf . ', ' . $idf . ', ' . $tf_idf . ', ' . $bm25 . ')';
			}
